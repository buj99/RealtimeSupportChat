...
//add emoji picker
const emojiPiker = document.createElement("script");
emojiPiker.src = "http://localhost:8090/static/js/emojiPickerSmallChat.js";
emojiPiker.type = "module";
document.body.parentNode.appendChild(emojiPiker);
...
let clientPhotoLink = window.localStorage.getItem(
  uniqueAdminToken + "_photo_link"
);
...
sendBtn.onclick = () => {
  ...
  if (window.localStorage.getItem(uniqueAdminToken + "_name") == null) {
    ...
    fetch(
      "http://localhost:3000/conversations/" + adminName + "/" + clientName,
      {
        method: "POST",
        headers: { auth_chat: authChat },
...
const removeCurrentMessages = () => {
  ...
  Array.from(document.getElementsByClassName("message moderator")).forEach(
    (message) => {
      message.remove();
    }
  );
