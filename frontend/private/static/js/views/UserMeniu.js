  ...
  createConversations(conversations) {
    ...
    if (this.conversations == undefined) {
      ...
      sortedConversations.forEach((conversation, index) => {
        if (
          (document.getElementsByClassName("conversation").length == 0 &&
            conversations.length != 0) ||
          (this.conversations[index] == undefined &&
            conversation.lastMsg.date != undefined) ||
    ...
    if (isThereANewMessage) {
      ...
      //debug
      let clientName = this.chatTitleElemen.innerText;
      if (clientName) {
        let admin = window.localStorage.getItem("admin");
        fetch(
          "http://localhost:3000/conversations/" + admin + "/" + clientName,
          {
            method: "GET",
            headers: { auth_chat: this.currentAuthChat },
          }
