...
const loginUser = async(pathParams, req, res) => {
    //verify if the corect method is used
    if (req.method != "POST") {
        res.writeHead(405, headers);
        return res.end(
...
const getUniqueChatKey = async(pathParams, req, res) => {
    ...
    if (verified.succes) {
        Admin.findById(verified.verified._id)
            .then((admin) => {
                res.writeHead(200, headers);
                res.end(JSON.stringify({ unique_admin_token: admin.unique_chat_key }));
            })
            .catch((error) => {
                console.log(error);
                res.writeHead(500, headers);
